...
class VoteAlgorithm:
    ...
    def load_algorithms(cls) -> list:
        ...
        if not cur_conn.table_exists('algorithm'):
            raise LookupError(f'There is no "algorithm" table in {cls._db_name} data base. Save some algorithm before load it.')
        ...
        for mdl in q_set:
            try:
                ...
                m_alg_name, m_func, m_name, m_code, m_pkg = mdl[1], mdl[2], mdl[3], mdl[4], mdl[5]
                ...
                except (ModuleNotFoundError, ModuleNotLoadedError, FunctionNotFoundInModuleError) as e:
