...
def process_clone_versions(
    module, clone_versions, cur_correct_val, result_lst, i, experiment_name
):
    try:
...
def process_similar_versions(
    module, similar_versions, cur_correct_val, result_lst, i, experiment_name
):
...
def process_partly_similar_versions(
    ...
    for ver1 in partly_similar_versions:
        for ver2 in partly_similar_versions:
            if ver1 != ver2:
                ...
                if (
                    module.normed_connectivity_matrix[v1_index][v2_index]
    ...
    correct_partly_similar_versions = {
        ver for ver in partly_similar_versions if random() <= ver.reliability
    }
    ...
    for ver in error_partly_similar_versions:
        partly_similar_error_val = generate_error_value(
            module, partly_similar_base_error_val, cur_partly_similar_diversity
        )
        result_lst.append(
            create_result(
                module,
...
def generate_experiment_data(
    module, iterations_amount: int, experiment_name: str
) -> list:
    ...
    for i in range(iterations_amount):
        ...
        process_partly_similar_versions(
            module,
            partly_similar_versions,
            cur_correct_val,
            result_lst,
